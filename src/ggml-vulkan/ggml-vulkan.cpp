...
struct vk_device_struct {
    ...
    vk_pipeline pipeline_repeat_f32, pipeline_repeat_f16, pipeline_repeat_back_f32;
...
using vk_op_conv2d_deform_push_constants = vk_op_conv2d_push_constants;
...
static vk_pipeline ggml_vk_op_get_pipeline(ggml_backend_vk_context * ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * dst, ggml_op op) {
    ...
    case GGML_OP_REPEAT:
        ...
        } else if (ggml_type_size(src0->type) == 2 && ggml_type_size(dst->type) == 2) {
    ...
    case GGML_OP_CONV_TRANSPOSE_2D:
        if (src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32 &&
            ...
            bool transpose = op == GGML_OP_CONV_TRANSPOSE_2D;
            ...
            uint32_t s1 = !transpose ? static_cast<uint32_t>(dst->op_params[1]) : static_cast<uint32_t>(dst->op_params[0]);
            uint32_t p0 = !transpose ? static_cast<uint32_t>(dst->op_params[2]) : 0;
            uint32_t p1 = !transpose ? static_cast<uint32_t>(dst->op_params[3]) : 0;
            uint32_t d0 = !transpose ? static_cast<uint32_t>(dst->op_params[4]) : 1;
            uint32_t d1 = !transpose ? static_cast<uint32_t>(dst->op_params[5]) : 1;
