...
void ggml_compute_forward_conv_2d_cwhn(
    ...
    if (knl_w == 1 && knl_h == 1 && stride_x == 1 && stride_y == 1) {
        ...
        ggml_call_mul_mat(params, m, c_out, c_in, src_data, knl_data, dst_data);
        return;
    }
...
static void ggml_compute_forward_upscale_f32(
    ...
    } else if ((mode & GGML_SCALE_MODE_MASK) == GGML_SCALE_MODE_BILINEAR) {
        float pixel_offset = 0.5f;
        if ((mode & GGML_SCALE_ALIGN_CORNERS) != 0){
            pixel_offset = 0.0f;
            sf0 = float(ne0 - 1) / float(src0->ne[0] - 1);
            sf1 = float(ne1 - 1) / float(src0->ne[1] - 1);
        }
...
static void ggml_compute_forward_roll_f32(
    ...
    for (int64_t i = start; i < end; ++i) {
        ...
        const int64_t dst_offset = (i3*nb3 + i2*nb2 + i1*nb1) / sizeof(float);
        ...
        const int64_t src_offset = (i03*nb03 + i02*nb02 + i01*nb01) / sizeof(float);

        const int64_t s = ggml_wrap_index(-s0, ne00);
        const int64_t n = ne00 - s;
        ggml_vec_cpy_f32(n, dst_data + dst_offset,     src_data + src_offset + s);
        ggml_vec_cpy_f32(s, dst_data + dst_offset + n, src_data + src_offset);
    }
}
